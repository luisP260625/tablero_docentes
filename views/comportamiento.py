...
def _agregar_otros_modulos_adeudados_pl(
    ...
    *,
    ...
    El cálculo se realiza por matrícula, dentro del mismo plantel y semana:

        módulos adicionales =
            módulos no competentes únicos del estudiante con todos los docentes
            - módulos no competentes únicos con el docente consultado

    Solo se agrega la cantidad. No se muestran nombres de otros docentes ni
    los módulos que imparten.

    Si no se encuentran las columnas necesarias para realizar el cálculo,
    se conserva la tabla y se agrega la columna con valores nulos para evitar
    ...
    # ------------------------------------------------------
    # Universo de comparación: mismo plantel y misma semana.
    # ------------------------------------------------------
    ...
    if col_semana_real and semana_actual is not None:
        universo = (
            ...
            .with_columns(
                ...
                .alias("_SEMANA_NUM_OTROS")
            )
            .filter(pl.col("_SEMANA_NUM_OTROS") == int(semana_actual))
            .drop("_SEMANA_NUM_OTROS")
        )
    ...
    # ------------------------------------------------------
    # Total de módulos únicos adeudados por cada estudiante.
    # ------------------------------------------------------
    total_por_estudiante = (
        universo
        .select(
            pl.col(col_matricula_real)
            .map_elements(_norm_value, return_dtype=pl.Utf8)
            .alias("_MATRICULA_NORM"),
            pl.col(col_modulo_real)
            .map_elements(_norm_value, return_dtype=pl.Utf8)
            .alias("_MODULO_NORM"),
        )
        .filter(
        ...
        .agg(
            pl.len().cast(pl.Int64).alias("_TOTAL_MODULOS_ADEUDADOS")
        )
    )

    # ------------------------------------------------------
    # Módulos únicos que el estudiante adeuda con el docente
    ...
    modulos_con_docente = (
        ...
        .group_by("_MATRICULA_NORM")
        .agg(
            pl.len().cast(pl.Int64).alias("_MODULOS_CON_DOCENTE")
        )
    )
    ...
    # ------------------------------------------------------
    # Agregar el resultado a cada registro visible del docente.
    # ------------------------------------------------------
    return (
        df_rep_docente
        .with_columns(
