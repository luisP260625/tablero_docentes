...
def mostrar(df, plantel_usuario, es_admin):
    ...
    df_docentes = df_modulo_ultima.group_by(["DOCENTE", "SEMESTRE","PCAPTURA"]).agg(
        pl.sum("NO COMPETENTES").alias("NO_COMP"),
        pl.sum("TOTAL ALUMNOS").alias("TOTAL")
    ).with_columns([
    ...
    df_docentes = df_docentes.select(["DOCENTE", "SEMESTRE","PCAPTURA","NO_COMP", "COMPETENTES", "TOTAL", "PORCENTAJE_NO_COMP"])
